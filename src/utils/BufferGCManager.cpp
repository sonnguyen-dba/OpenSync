...
void BufferGCManager::shrinkBuffersIfNeeded() {
    ...
    for (const auto& [tableKey, lastFlush] : lastFlushRef) {
	    ...
	    Logger::info("🧹 Shrunk buffer for inactive table: " + tableKey);
    ...
    if (shrinkCount > 0) {
	Logger::debug("🔄 Shrink cycle complete. Total tables shrunk: " + std::to_string(shrinkCount));
    }
}
...
void BufferGCManager::start(std::unordered_map<std::string, std::pair<std::vector<std::string>, std::chrono::steady_clock::time_point>>& tableSQLBuffer,
    ...
    std::thread([&]() {
        while (true) {
		    ...
		    Logger::info("♻️ GC table buffer: " + table + ", elapsed=" + std::to_string(elapsed) + "s, size=" + std::to_string(it->second.first.size()));
